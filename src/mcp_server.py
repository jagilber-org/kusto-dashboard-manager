...
from tracer import trace_error, trace_func_entry, trace_func_exit, trace_info
...
class KustoDashboardMCPServer:
    ...
    async def _import_dashboard(
        self, url: str, json_path: str, force: bool = False
    ) -> Dict:
    ...
    async def _export_all_dashboards(
        self, snapshot_yaml: str, creator_filter: str = None
    ) -> Dict:
    ...
    async def _parse_dashboards_from_snapshot(
        self, snapshot_yaml: str, creator_filter: str = None
    ) -> Dict:
        """Parse dashboard list from Playwright snapshot YAML - NO BROWSER ACCESS NEEDED"""
        trace_func_entry(
            "_parse_dashboards_from_snapshot",
            creator_filter=creator_filter,
            yaml_length=len(snapshot_yaml),
        )
        try:
            from dashboard_export import DashboardExporter
            ...
            trace_func_exit(
                "_parse_dashboards_from_snapshot",
                result=f"{len(dashboards)} dashboards",
            )
            return result
        except Exception as e:
    ...
    async def handle_request(self, request: Dict) -> Dict:
        ...
        try:
            ...
            if method == "initialize":
                ...
                return self._create_response(
                    req_id, {"tools": self._get_tool_definitions()}
                )
                ...
                if tool_name == "parse_dashboards_from_snapshot":
                    ...
                    result = await self._validate_dashboard(arguments["jsonPath"])
                    ...
                    trace_info(
                        "export_all_dashboards completed",
                        found=result.get("total_found"),
                    )
                else:
            ...
            return self._create_response(
                request.get("id"), error=self._create_error(-32603, str(e))
            )
            ...
            while True:
                ...
                try:
                    ...
                    error_response = self._create_response(
                        None, error=self._create_error(-32700, "Parse error")
                    )
