...
class Config:
    ...
    def _init_tracing(self):
        ...
        trace_enabled = self.data.get("TRACE_ENABLED", "").lower() in (
            "true",
            "1",
            "yes",
        ) or self.get("tracing.enabled", False)

        print(f"[DEBUG] trace_enabled: {trace_enabled}", file=sys.stderr)

        if trace_enabled:
            level_str = self.data.get("TRACE_LEVEL") or self.get(
                "tracing.level", "DEBUG"
            )
            ...
            log_dir = self.data.get("TRACE_LOG_DIR") or self.get(
                "tracing.log_dir", "logs"
            )

            print(
                f"[DEBUG] Enabling tracing: file_logging={file_logging}, level={level_str}, log_dir={log_dir}",
                file=sys.stderr,
            )
    ...
    def validate(self):
        ...
        if browser_type not in valid_browsers:
            raise ValueError(
                f"Invalid browser type: {browser_type}. Must be one of {valid_browsers}"
            )
    ...
    def from_env(cls):
        ...
        if "KDM_HEADLESS" in os.environ:
            browser["headless"] = os.environ["KDM_HEADLESS"].lower() in (
                "true",
                "1",
                "yes",
            )
