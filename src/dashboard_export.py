...
from tracer import trace_func_entry, trace_func_exit, trace_info, trace_error, trace_parse, trace_browser_action
...
class DashboardExporter:
    ...
    async def _get_dashboard_list(self, list_url):
        ...
        print_info(f"Snapshot result type: {type(snapshot_result)}, has content: {bool(snapshot_result)}")
        
        # Save snapshot to file for debugging
        snapshot_file = "dashboard_list_snapshot.json"
        with open(snapshot_file, 'w', encoding='utf-8') as f:
            json.dump(snapshot_result if snapshot_result else {"error": "No result"}, f, indent=2)
        ...
        if isinstance(snapshot_result, dict) and "raw" in snapshot_result:
            ...
            trace_error("Unexpected snapshot format", format_type=type(snapshot_result).__name__)
            self.logger.error(f"Unexpected snapshot format: {snapshot_result.keys() if isinstance(snapshot_result, dict) else type(snapshot_result)}")
    ...
    def _parse_raw_snapshot_text(self, raw_text: str, dashboards: list):
        ...
        while i < len(lines):
            ...
            if row_match:
                row_text = row_match.group(1)  # e.g., "armprod about 1 hour ago 11/3/2020 Jason Gilbertson"
                ...
                for j in range(i+1, min(i+10, len(lines))):
                    ...
                    if url_match:
                        ...
                        url = f"https://dataexplorer.azure.com/dashboards/{dashboard_id}"
        ...
        trace_func_exit("_parse_raw_snapshot_text", result=f"{len(dashboards)} dashboards")
    ...
    def _parse_dashboard_nodes(self, node, dashboards, current_dashboard=None):
        ...
        if isinstance(node, dict):
            ...
            if role == "text" and current_dashboard and not current_dashboard.get("creator"):
                # Check if this text looks like a creator name
                if any(indicator in name.lower() for indicator in ["by ", "created by", "@"]):
                    current_dashboard["creator"] = name.replace("by ", "").replace("created by ", "").strip()
    ...
    async def export_all_dashboards(self, list_url="https://dataexplorer.azure.com/dashboards", creator_filter=None):
        """
        ...
        """
        trace_func_entry("export_all_dashboards", list_url=list_url, creator_filter=creator_filter)
        ...
        try:
            ...
            if creator_filter:
                ...
                trace_info("Filtered dashboards", original=original_count, filtered=len(dashboards), creator=creator_filter)
                self.logger.info(f"Filtered to {len(dashboards)} dashboards by creator: {creator_filter}")
            ...
            for i, dashboard in enumerate(dashboards, 1):
                url = dashboard["url"]
                trace_info(f"Exporting dashboard {i}/{results['filtered']}", url=url, name=dashboard.get('name'))
            ...
            # Summary
            trace_info("Bulk export complete", exported=results['exported'], failed=results['failed'])
            ...
            trace_func_exit("export_all_dashboards", result=f"{results['exported']} exported, {results['failed']} failed")
            return results
